...
let sets={sei:true,daku:false,you:false}, scripts={hira:true,kata:false}, mode="choose";
let autoSpeak=true, slow=false, theme="sakura", jaVoice=null;
let C=0,W=0,streak=0,best=0,miss={},cur=null,locked=false;
const $=id=>document.getElementById(id);
const root=document.documentElement;
const SAVE="kanaSave_v2";
function save(){try{localStorage.setItem(SAVE,JSON.stringify({C,W,best,miss,sets,scripts,mode,autoSpeak,slow,theme}));}catch(e){}}
function load(){try{const d=JSON.parse(localStorage.getItem(SAVE));if(!d)return;C=d.C||0;W=d.W||0;best=d.best||0;miss=d.miss||{};if(d.sets)sets=d.sets;if(d.scripts)scripts=d.scripts;if(d.mode)mode=d.mode;autoSpeak=d.autoSpeak!==false;slow=!!d.slow;theme=d.theme||"sakura";}catch(e){}}
...
function pick(){const p=allFor();$("praise").textContent="";$("praise").className="praise";if(!p.length){$("q").textContent="…";$("area").innerHTML="<div class=foot>至少选一个『考哪些』和一种假名喵</div>";return;}
  $("next").style.display="none";$("card").classList.remove("pop","glow");cur=p[Math.floor(Math.random()*p.length)];locked=false;render();if(autoSpeak)speak(cur[0]);}
...
function judge(ok,el,kind){locked=true;const card=$("card");
  if(ok){C++;streak++;if(streak>best)best=streak;$("fb").textContent="✓ 对啦";$("fb").className="fb good";if(el)el.classList.add("good");
  ...
  else{W++;streak=0;const key=cur[0]+" "+cur[1];miss[key]=(miss[key]||0)+1;
